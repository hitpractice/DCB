...
public class UserLoginActivity extends Activity implements OnClickListener {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                if (!hasFocus) {
                    ...
                    if (username.length() < 4) {
                        Toast.makeText(UserLoginActivity.this, "用户名不能小于4个字符",
                                Toast.LENGTH_SHORT);
                    }
                }
            }
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                if (!hasFocus) {
                    ...
                    if (password.length() < 4) {
                        Toast.makeText(UserLoginActivity.this, "密码不能小于4个字符",
                                Toast.LENGTH_SHORT);
                    }
                }
            }
    ...
    private boolean checkEdit() {
        if (login_username.getText().toString().trim().equals("")) {
            Toast.makeText(UserLoginActivity.this, "用户名不能为空", Toast.LENGTH_SHORT)
                    .show();
    ...
    private class OnClickListenerImpl implements OnClickListener {
        ...
        public void onClick(View arg0) {
            ...
            database = new MyDatabaseHelper(UserLoginActivity.this)
                    .getReadableDatabase();
            String user2 = UserLoginActivity.this.login_username.getText().toString();
            String password = UserLoginActivity.this.login_password.getText()
                    .toString();
            ...
            if (cursor.getCount() > 0) {
                ...
                if (result.equals(user2)) {
                    Toast myToast = Toast.makeText(UserLoginActivity.this, "登录成功",
                            Toast.LENGTH_SHORT);
                    myToast.setGravity(Gravity.CENTER, 60, 30);
                    LinearLayout myToastView = (LinearLayout) myToast.getView();
                    ImageView img = new ImageView(UserLoginActivity.this);
                    img.setImageResource(R.drawable.successlogo);

                    //一致，成功登陆
                } else {
                    Toast myToast = Toast.makeText(UserLoginActivity.this, "登录失败",
                            Toast.LENGTH_SHORT);
                    myToast.setGravity(Gravity.CENTER, 60, 30);
                    LinearLayout myToastView = (LinearLayout) myToast.getView();
                    ImageView img = new ImageView(UserLoginActivity.this);
